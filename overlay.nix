...
in {
  ...
  # difficulties with GFortran build
  gfortran = null;
  ...
  lapack = prev.lapack.override { lapackProvider = final.blis; };
  ...
  perlPackages = prev.perlPackages.overrideScope (final: prev: {
    BCOW = prev.BCOW.overrideAttrs (prevAttrs: {
      ...
      makeMakerFlags = prevAttrs.makeMakerFlags or [ ] ++ [ "cc=${lib.getExe' stdenv.cc (targetPrefix + "cc")}" ];
    });
  ...
  bat = prev.bat.overrideAttrs (prevAttrs: {
    nativeCheckInputs = prevAttrs.nativeCheckInputs or [ ] ++ [ stdenv.cc ];
  });
  ...
  helix = prev.helix.overrideAttrs (prevAttrs: {
    nativeCheckInputs = prevAttrs.nativeCheckInputs or [ ] ++ [ stdenv.cc ];
  });
  ...
  keyutils = prev.keyutils.overrideAttrs (prevAttrs: {
    NIX_LDFLAGS = prevAttrs.NIX_LDFLAGS or [ ] ++ [ "--undefined-version" ];
  });
  ...
  makeBinaryWrapper = prev.makeBinaryWrapper.overrideAttrs (prevAttrs: {
    cc = let inherit (final.targetPackages.stdenv) cc;
    in prevAttrs.cc + lib.optionalString cc.isClang "--ld-path=${lib.getExe' cc.bintools (cc.bintools.targetPrefix + "ld")} ";
  });
  ...
  xsimd = prev.xsimd.overrideAttrs {
    doCheck = false;
  };
